...
class imgPortofolio extends React.Component {
  showLearnDetail = (action, val) => {
    switch (action) {
      case "show": {
        document.getElementById(`show_backimg${val}`).style.cssText =
          "width:100%; transition:.5s;";
        document.getElementById(`show_name${val}`).style.cssText =
  ...
  render() {
    return (
      <div
        ...
        id={"porto" + this.props.index}
        onMouseOver={() => this.showLearnDetail("show", this.props.index)}
        onMouseOut={() => this.showLearnDetail("hide", this.props.index)}
      ...
      >
        ...
        />
        <div
          className="bg-black h-full absolute top-0 opacity-60 w-0"
          id={`show_backimg${this.props.index}`}
        ></div>{" "}
        {/* background black */}
        <div
          className="flex flex-col justify-center items-center h-full w-full absolute top-0 z-20 text-gray-100 overflow-hidden opacity-0"
          id={`show_name${this.props.index}`}
        >
          <h2 className="font-semibold text-2xl">{this.props.name}</h2>
          <p className="text-lg text-blue-100">{this.props.framework}</p>
          <button
            className="mt-14 w-1/3 border-2 border-blue-500 py-2 text-lg hover:bg-blue-500"
            onClick={() =>
              this.openModal(
                this.props.img_full,
                this.props.about,
                this.props.button_type,
                this.props.site,
                this.props.download,
                this.props.github,
              )
